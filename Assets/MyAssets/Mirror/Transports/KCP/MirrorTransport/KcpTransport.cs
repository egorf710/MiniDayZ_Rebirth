...
namespace kcp2k
{
    ...
    public class KcpTransport : Transport
    {
        ...
        public Vector4 StatisticsWindow = new Vector4(5, 110, 300, 400);
        public int FontSize;
        void OnGUI()
        {
            if (!statisticsGUI) return;

            GUILayout.BeginArea(new Rect(StatisticsWindow.x, StatisticsWindow.y, StatisticsWindow.z, StatisticsWindow.w));
            ...
            if (ServerActive())
            {
                ...
                GUILayout.Label("SERVER", headStyle);
                GUILayout.Label($"  kcp SERVER @ time: {(int)NetworkTime.localTime}", headStyle);
                GUILayout.Label($"  connections: {server.connections.Count}", headStyle);
                GUILayout.Label($"  MaxSendRate (avg): {PrettyBytes(GetAverageMaxSendRate())}/s", headStyle);
                GUILayout.Label($"  MaxRecvRate (avg): {PrettyBytes(GetAverageMaxReceiveRate())}/s", headStyle);
                GUILayout.Label($"  SendQueue: {GetTotalSendQueue()}", headStyle);
                GUILayout.Label($"  ReceiveQueue: {GetTotalReceiveQueue()}", headStyle);
                GUILayout.Label($"  SendBuffer: {GetTotalSendBuffer()}", headStyle);
                GUILayout.Label($"  ReceiveBuffer: {GetTotalReceiveBuffer()}", headStyle);
            ...
            {
                GUILayout.BeginVertical("Box");
                GUILayout.Label("CLIENT", headStyle);
                GUILayout.Label($"  kcp CLIENT @ time: {(int)NetworkTime.localTime}", headStyle);
                GUILayout.Label($"  MaxSendRate: {PrettyBytes(client.peer.MaxSendRate)}/s", headStyle);
                GUILayout.Label($"  MaxRecvRate: {PrettyBytes(client.peer.MaxReceiveRate)}/s", headStyle);
                GUILayout.Label($"  SendQueue: {client.peer.SendQueueCount}", headStyle);
                GUILayout.Label($"  ReceiveQueue: {client.peer.ReceiveQueueCount}", headStyle);
                GUILayout.Label($"  SendBuffer: {client.peer.SendBufferCount}", headStyle);
                GUILayout.Label($"  ReceiveBuffer: {client.peer.ReceiveBufferCount}", headStyle);
                GUILayout.EndVertical();
            }
